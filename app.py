...
import time
import base64
import qrcode  # 确保安装了 qrcode 库：pip install qrcode[pil]
...
PORT = int(os.environ.get('PORT_59157') or os.environ.get('PORT') or 27017)  # 订阅端口，游戏玩具类若无法订阅可改为分配的端口
...
def generate_config():
    config = {
        ...
        "inbounds": [
            {"port": ARGO_PORT, "protocol": "vless", "settings": {"clients": [{"id": UUID, "flow": "xtls-rprx-vision", }, ],
                                                                  "decryption": "none",
                                                                  "fallbacks": [{"dest": 3001 },
        ...
        "outbounds": [
            ...
            {"tag": "WARP", "protocol": "wireguard",
             "settings": {"secretKey": "YFYOAdbw1bKTHlNNi+aEjBM3BO7unuFC5rOkMRAz9XY=",
                           ...
                           "peers": [{"publicKey": "bmXOC+F1FxEMF9dyiK2H5/1SUtzH0JuVo51h2wPfgyo=",
                                      "allowedIPs": ["0.0.0.0/0", "::/0"], "endpoint": "162.159.193.10:2408"}],
...
def generate_links(argo_domain):
    ...
    VMESS = {"v": "2", "ps": f"{NAME}-{ISP}", "add": CFIP, "port": CFPORT, "id": UUID, "aid": "0", "scy": "none",
             "net": "ws", "type": "none", "host": argo_domain, "path": "/vmess?ed=2048", "tls": "tls", "sni": argo_domain,
             "alpn": ""}
    list_txt = f"""
...
trojan://{UUID}@{CFIP}:{CFPORT}?security=tls&sni={argo_domain}&type=ws&host={argo_domain}&path=%2Ftrojan?ed=2048#{NAME}-{ISP}
    ...
    try:
        ...
        qr.add_data(sub_txt)  # 将订阅链接作为二维码数据
        qr.make(fit=True)
        img = qr.make_image(fill_color="black", back_color="white")
        qr_code_path = os.path.join(FILE_PATH, 'sub_qr.png')  # 保存二维码图片路径
